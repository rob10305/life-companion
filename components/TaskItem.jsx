...
export default function TaskItem({ task, onEdit }) {
  ...
  return (
    <div className={`group flex items-start gap-3 px-4 py-3 rounded-xl border transition-all ${
      task.completed
        ? 'border-transparent bg-cream-200/60 dark:bg-gray-900/40 opacity-50'
        : 'border-cream-300 dark:border-gray-800 bg-white dark:bg-gray-900 hover:border-cream-400 dark:hover:border-gray-700'
    }`}>
      {/* Checkbox */}
      <button
        ...
        className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
          task.completed
            ...
            : 'border-cream-400 dark:border-gray-600 hover:border-cream-400 dark:hover:border-gray-400'
        }`}
      >
        {task.completed && (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start gap-2 flex-wrap">
          <span className={`text-sm leading-5 ${task.completed ? 'line-through text-notion-muted dark:text-gray-500' : 'text-notion-text dark:text-white'}`}>
            {task.title}
          </span>
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          {dateLabel && (
            <span className={`flex items-center gap-1 text-xs ${overdue ? 'text-red-400' : 'text-notion-muted dark:text-gray-500'}`}>
              <Calendar size={10} />
              {dateLabel}
            </span>
          )}
